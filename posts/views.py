...
from .models import Post, Comment, Question, Vote
from .forms import NewPostForm, NewQuestionForm
...
def forum(request):
    ...
    return render(request, 'posts/forum.html', context=context)


def get_quests(request, num_posts):
    if not request.user.is_authenticated:
        return redirect('login')
    if request.is_ajax():
        visible = 10
        ...
        for post in posts:
            ...
            answers = []
            for i in post.answers: 
                answers.append({'answer_id': i.id, 'answer_content': i.content, 'answer_right': Profile.objects.filter(user=request.user).first() in i.users.all(), 'answer_right_count': len(i.users.all())})
...
def answer(request):
    ...
    if request.is_ajax():
        pk = request.POST.get('pk')
        ans = Vote.objects.get(id=pk)
        profile = Profile.objects.filter(user=request.user).first()

        print(ans.users.all(), " - ", profile)

        if profile in ans.users.all():
            ans.users.remove(profile)
            answed = False
        else:
            ans.right(profile)
            answed = True
        return JsonResponse({'liked': answed, 'count': len(ans.users.all())})
...
def new_quest(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            quest.author = Profile.objects.filter(user=request.user).first()
            quest.save()

            Vote.objects.create(content =answer1, question=quest)
            Vote.objects.create(content =answer2, question=quest)
            
            messages.success(request, 'New Question has been posted')
    ...
    return render(request, 'posts/new_quest.html', context=context)
